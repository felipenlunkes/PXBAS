/*
 * 
 *  Interpretador BASIC PXBAS para PX-DOS
 * 
 * Versão 0.1.1 Beta
 * 
 * Versão de testes 0
...
void nivel2(), nivel3(), nivel4(), nivel5(), nivel6(), primitivo();
void unary(), arith();
...
char *argv[];
{
  ...
  if(argc!=2) {
    printf("\nInterpretador PX-DOS Basic versao %s\n", VERSAO);
	printf("\n\nUse o Interpretador para executar programas PX-DOS Basic\n");
	printf("previamente construidos. O Interpretador cuida, para voce, da memoria,\n");
	printf("das alocacoes e de tudo que pode te distrair, para criar e executar\n");
	printf("programas faceis e simples de se utilizar.\n\n");
	printf("Interpretador PX-DOS Basic e Linguagem PX-DOS Basic.\n");
	printf("Copyright (C) 2013 Felipe Miguel Nery Lunkes\n\n");
	printf("\nUso: pxbas <programa.bas>\n\n");
    sair(1);
  }

  /* Aloca memória para o programa */
  if(!(p_buf=(char *) malloc(TAMANHO_PROG))) {
    printf("\n\nInterpretador PX-DOS Basic\n");
...
int erro;
{
  static char *e[]= {   
    "Erro de Sintaxe.", 
    "Paranteses nao balanceados.", 
    "Nenhuma expressao presente.",
    "Simbolo de igualdade esperado.",
    "Nao e uma variavel valida ou declarada.",
    "Lista de rotulos muito cheia.",
    "Rotulo duplicado encontrado.",
    "Rotulo nao definido chamado.",
    "ENTAO esperado.",
    "DEST esperado.",
	...
	"Erro ao iniciar o som."
	"Erro ao executar o beep."
	"Falha interna."
	"Erro geral de alocacao."
	"Reinicie seu computador."
	"A maquina virtual Basic do PX-DOS foi requisitada de forma invalida."
  }; 
  printf("%s\n", e[erro]); 
